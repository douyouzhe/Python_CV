...
results,listInPairs = match_features.match_features(featureList1,featureList2,pic1,pic2,60)
...
# featureList1 = detect_features.detect_features(pic1,20,0.25)
# featureList2 = detect_features.detect_features(pic2,20,0.17)
# results,listInPairs = match_features.match_features(featureList1,featureList2,pic1,pic2,10)

# pic1 = cv2.imread('wall2.png')
# pic2 = cv2.imread('wall3.png')
# featureList1 = detect_features.detect_features(pic1,20,0.4)
...
for i in range(0,len(results),2):
    ...
    cv2.line(newImage,(results[i][1],results[i][0]),(results[i+1][1]+offSet,results[i+1][0]),(color))
    cv2.circle(newImage,(results[i][1],results[i][0]),5,(color))
    cv2.circle(newImage, (results[i+1][1]+offSet,results[i+1][0]), 5, (color))
